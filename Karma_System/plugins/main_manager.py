...
from Karma_System import Karma_logs, ENFORCERS, Karma, INSPECTORS
from Karma_System.strings import (
    scan_request_string,
    reject_string,
    proof_string,
    forced_scan_string,
...
from Karma_System.utils import seprate_flags, Flag
...
async def punish(event, flags):
    ...
    if flags.o:
        if replied.fwd_from:
            ...
            if reply.from_id.user_id in ENFORCERS or reply.from_id.user_id in Karma:
                return
...
async def reject(event):
    ...
    if replied.from_id.user_id == me.id:
        ...
        if match:
            # print('Matched OmU')
            id = replied.id
            await System.edit_message(Karma_logs, id, reject_string)
    orig = re.search(r"t.me/(\w+)/(\d+)", replied.text)
