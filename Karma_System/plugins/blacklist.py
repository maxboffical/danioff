from Karma_System import System, Karma, ENFORCERS, Karma_logs, system_cmd
import re
import Karma_System.plugins.Mongo_DB.message_blacklist as db
import Karma_System.plugins.Mongo_DB.name_blacklist as wlc_collection
...
async def auto_gban_request(event):
    System.processing += 1
    if event.from_id.user_id in ENFORCERS or event.from_id.user_id in Karma:
        return
    if event.chat_id == Karma_logs:
        return
    text = event.text
    ...
    if words:
        for word in words:
            ...
            if re.search(pattern, text, flags=re.IGNORECASE):
                ...
                await System.send_message(Karma_logs, logmsg)
                System.processed += 1
                System.processing -= 1
                return
    System.processed += 1
...
async def auto_wlc_gban(event):
    ...
    if user.id in ENFORCERS or user.id in Karma:
        return
