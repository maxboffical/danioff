...
from Karma_System import ENFORCERS, INSPECTORS, Karma, session
...
json_file = os.path.join(os.getcwd(), "Karma_System\elevated_users.json")
...
async def addenf(event) -> None:
    ...
    if HEROKU:
        ...
        if not event.from_id.user_id in Karma:
